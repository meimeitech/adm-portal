...
//          }
    ...
    methods: {
      ...
      init: async function () {
        await userList({'pageNumber': this.pageNumber, 'pageSize': this.pageSize}, this.user, this.$store.state.app.baseUrl).then(r => {
      ...
      query: async function () {
        let res = await userList({'pageNumber': this.pageNumber, 'pageSize': this.pageSize}, this.user, this.$store.state.app.baseUrl);
      ...
      passReset: async function (authId, userId) {
        ...
        await passResest({'id': userId}, this.user, this.$store.state.app.baseUrl).then(r => {
      ...
      lockUser: async function(id) {
        await lockUser({'id': id}, this.$store.state.app.baseUrl).then(r => {
          let resultCode = r.header.code;
          if (resultCode === '0') {
      ...
      handleSubmit(name) {
        this.$refs[name].validate(async (valid) => {
          if (valid && name === 'formValidate') {
            await saveUser(this.formValidate, this.$store.state.app.baseUrl).then(r => {
              let resultCode = r.header.code;
              if (resultCode === '0') {
          ...
          if (valid && name === 'userEdit') {
            await updateUser(this.userEdit, this.$store.state.app.baseUrl).then(r => {
              let resultCode = r.header.code;
              if (resultCode === '0') {
